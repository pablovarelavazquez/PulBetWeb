...
<div id="divbanco">
	...
	<div class="green">
				<h2>
					<fmt:message key="ingresar" bundle="${messages}" />
				</h2>
	</div>
		...
		<form action="<%=ControllerPaths.USUARIO%>" method="post">
			...
			<input type="hidden" name="<%=ParameterNames.ACTION%>"
				value="<%=Actions.INGRESAR%>" /> <label><fmt:message
			...
			<c:forEach var="e" items="${parameterErrors}">
				<li class="fallada" class="fallada"><fmt:message key="${e}" bundle="${messages}" /></li>
			</c:forEach>

			<label><fmt:message key="cantidad" bundle="${messages}" /> </label>
			<input class="my-input" type="text" placeholder="1.0"
			...
			<br> <input class="my-input" type="submit"
				value="<fmt:message key="ingrbttn" bundle="${messages}"/>">
